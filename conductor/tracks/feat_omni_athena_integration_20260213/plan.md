...
- [ ] Task: Write Tests: Define expected signature/skeleton output for a sample C# project.
- [ ] Task: Implement: Port `SkeletonExtractor.cs` logic to a new Python module in `Omni.Athena`.
- [ ] Task: Write Tests: Verify Git history analysis logic in Python (parity with `GitHistoryService.cs`).
- [ ] Task: Implement: Port `GitHistoryService.cs` logic to Python.
- [ ] Task: Conductor - User Manual Verification 'Phase 2: Python-Powered OPC Porting (Signature Extraction)' (Protocol in workflow.md)
...
- [ ] Task: Write Tests: Verify a unified search that queries both session logs and codebase signatures.
- [ ] Task: Implement: Create `opc.py` entry point in `Omni.Athena` that orchestrates indexing and memory tools.
- [ ] Task: Write Tests: Verify `opc_sync` functionality (updating both context and memory).
- [ ] Task: Implement: Add `sync` command to the unified Python `opc` tool.
- [ ] Task: Conductor - User Manual Verification 'Phase 3: Unified opc Entry Point & Memory Integration' (Protocol in workflow.md)
...
- [ ] Task: Write Tests: Verify that `build_run_omnihub.py` is correctly identified as a "skill" by the agent.
- [ ] Task: Implement: Build a skill discovery mechanism in `Omni.Athena` that scans the Omni root for common scripts.
- [ ] Task: Implement: Final project-wide `opc sync` and memory audit.
- [ ] Task: Conductor - User Manual Verification 'Phase 5: Automated Skill Injection & Final Verification' (Protocol in workflow.md)
